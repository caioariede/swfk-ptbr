...
\noindent
1. Existe uma forma difícil e uma fácil de desenhar um octógono. A forma difícil não é difícil por ser complicada, é difícil pois requer muita digitação:
...
\noindent
2. se você olhar as outras funções, do Capítulo~\ref{ch:turtlesgalore}, você verá como criar uma forma preenchida. Nós podemos mover o código do octógono para uma função que recebe uma cor.
...
Nós especificamos a cor e então ligamos o preenchimento (\code{begin\_fill}). Então nós executamos o laço `for' para desenhar o octógono e finalmente desligamos o preenchimento (\code{end\_fill}). Que tal um octógono azul? Como no da figura ~\ref{fig49}:
